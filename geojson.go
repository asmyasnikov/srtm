...
func AddElevations(tileDir string, storeInMemoryMode bool, geoJson *geojson.Geometry, skipErrors bool) (*geojson.Geometry, error) {
	switch geoJson.Type {
	...
	case geojson.GeometryMultiLineString:
		...
		for i := range geoJson.MultiLineString {
			...
			for j, point := range geoJson.MultiLineString[i] {
				go func(i, j int) {
					point, err := AddElevation(tileDir, storeInMemoryMode, point)
					if err != nil && !skipErrors {
						log.Error().Caller().Err(err).Msg("")
					}
